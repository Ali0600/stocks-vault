...
and the snapshot-freshness check warns (never fails) so a dead daily refresh
surfaces in CI/local lint without breaking the gate.

The checks live in `lint(root)`, which returns (fails, warns, counts) without
printing or exiting, so they can be unit-tested against a fixture vault. The
`__main__` block runs it against the repo root and owns all printing + exit codes.
"""
import re, glob, os, sys, datetime

DEFAULT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SNAPSHOT_MAX_AGE_DAYS = 7  # daily refresh → older than a week means the job stalled
...
def lint(root):
    """Run all structural checks against the vault at `root`.

    Returns (fails, warns, counts) where counts = (stock_notes, concepts, articles).
    Does not print or exit — the CLI wrapper owns that.
    """
    os.chdir(root)
    ...
    for f in stock_files:
        ...
        if "<!-- snapshot:start -->" not in txt or "<!-- snapshot:end -->" not in txt:
            F(f"{t}: missing Snapshot block markers")
        else:
            # snapshot freshness (warn-only heartbeat for the daily refresh job)
            am = re.search(r'_Auto-updated (\d{4}-\d{2}-\d{2}) via yfinance\._', txt)
            if not am:
                W(f"{t}: snapshot block present but no parseable Auto-updated date")
            else:
                try:
                    age = (today - datetime.date.fromisoformat(am.group(1))).days
                    if age > SNAPSHOT_MAX_AGE_DAYS:
                        W(f"{t}: snapshot stale — Auto-updated {am.group(1)} ({age}d ago, > {SNAPSHOT_MAX_AGE_DAYS}d); is the daily refresh running?")
                except ValueError:
                    W(f"{t}: snapshot has an unparseable Auto-updated date: {am.group(1)}")
        for sec in DATED:
    ...
    counts = (len(stock_files), len(glob.glob("Concepts/*.md")), len(glob.glob("Articles/*.md")))
    return fails, warns, counts


if __name__ == "__main__":
    fails, warns, (n_stock, n_concept, n_article) = lint(DEFAULT_ROOT)
    print(f"Stocks Vault lint — {n_stock} stock notes, {n_concept} concepts, {n_article} articles")
    if warns:
        print(f"\n{len(warns)} warning(s):")
