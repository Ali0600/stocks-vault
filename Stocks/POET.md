---
ticker: POET
sector: Networking & Optical
updated: 2026-06-12
---
# POET Technologies Inc. (POET)

## Overview
POET Technologies develops optical interposers and photonic engines for AI interconnect — earlier-stage and more speculative than AAOI, with most of the value resting on design wins converting to volume revenue. ~$1.9B market cap.

## Snapshot
<!-- snapshot:start -->
| Mkt cap | Revenue (TTM) | Rev growth | Net margin | P/E | Next earnings |
| --- | --- | --- | --- | --- | --- |
| $1B | $2M | +112% | 0% | n/m | 2026-11-12 |

_Auto-updated 2026-09-05 via yfinance._
<!-- snapshot:end -->

## What's Unique
The Optical Interposer platform integrates lasers and optics at wafer scale, aiming to strip cost out of [[Optical interconnect|AI-interconnect]] packaging — a differentiated architecture, though value still hinges on design wins converting to volume.

### Only They Do
- Patented Optical Interposer platform integrating lasers and optics at wafer scale (company IP)
- **Chokepoint:** Low — a differentiated, patented architecture, but pre-volume and unproven; design wins have not converted into gating supply

## Competitors
- [[AAOI]] — incumbent transceiver volume
- [[LITE]] / [[COHR]] — established optical components
- [[INTC]] — silicon photonics
- [[MRVL]] — optical DSP and integration

## Supply Chain
### Suppliers
- Wafer/foundry & packaging partners (external)

### Customers
- Transceiver-module makers & OEMs (external) — optical-engine design wins (pre-volume)

## Facts

## Contracts Awarded

## Dilutions

## Speculations

## Open Questions

## Articles
